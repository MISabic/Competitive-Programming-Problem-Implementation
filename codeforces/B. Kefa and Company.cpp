...
double c_dist(double x1,double y1,double x2,double y2)
{
...
double trngl_area2(double x1,double y1,double x2,double y2,double x3,double y3)
{
...
int main()
{
    ...
    //cin.tie(0);
    ll t,n,d,m,p,q,r,s,k,u,v,sum[100100];
    while(cin>>n>>d)
    {
        ll mx=0,res=0,j=1;
        loopi(i,1,n)
        {
            cin>>arr[i].fi>>arr[i].si;
        }
        sort(arr,arr+n+1);
        sum[0]=0;
        loopi(i,1,n)
        {
            sum[i]=sum[i-1]+arr[i].si;
        }
        loopi(i,1,n)
        {
            while(arr[i].fi>=arr[j].fi+d){
                ++j;
            }
            mx=max(mx,sum[i]-sum[j-1]);
        }
        cout<<mx<<endl;
    }
    return 0;
