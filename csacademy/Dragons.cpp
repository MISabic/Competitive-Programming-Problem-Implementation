...
int main()
{
    ...
    cin.tie(0);
    int t,n,m,k,p,q,r,s,u,v,arr[100][100];
    while(cin>>n>>m){
        int res[100][100];
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
        ...
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
                if(arr[i][j]==1){
                    ...
                    while(!q.empty()){
                        int x=q.front().fi;
                        int y=q.front().si;//cout<<x<<"    "<<y<<endl;
                        q.pop();
                        for(int k=0; k<8; k++){
                            if(isValid(n,m,x+fx2[k],y+fy2[k]) && visited[x+fx2[k]][y+fy2[k]]==0){
                                visited[x+fx2[k]][y+fy2[k]]=1;
                                q.push(mkp(x+fx2[k],y+fy2[k]));
                                if(res[x+fx2[k]][y+fy2[k]]==0){
                                    res[x+fx2[k]][y+fy2[k]]=res[x][y]+1;
                                }
                                else res[x+fx2[k]][y+fy2[k]]=min(res[x+fx2[k]][y+fy2[k]],res[x][y]+1);
                            }
                        }
                    }
                }
